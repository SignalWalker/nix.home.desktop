            ...
            fn = let mkFn = fn: mkOption {
            ...
            }; in
              {
                ...
                exec =
                  ...
                  in
                  mkFn (if config.startup == null then null else ("'${notif "Starting ${config.name}"} && ${config.startup} || ${notif "Failed to start ${config.name}"}'"));
              };
          };
          config = { };
        })
...
in
{
  ...
  config = {
    signal.desktop.wayland.compositor.scratchpads = [
      { kb = "Grave"; criteria = { app_id = "scratch_term"; }; resize = 83; startup = "kitty --class scratch_term"; }
      ...
      { kb = "Shift+T"; criteria = { app_id = "scratch_top"; }; resize = 83; startup = "kitty --class scratch_top btop"; }
      { kb = "Shift+Slash"; criteria = { app_id = "org.kde.dolphin"; }; resize = 83; startup = "dolphin"; }
    ];
