      ...
      in
      {
        ...
        config = {
          ...
          floating = {
            ...
            border = 1;
            criteria = foldl' (
              acc: window: if window.floating then acc ++ [ window.criteria ] else acc
          ...
          keybindings =
            ...
            // (foldl'
              (
                acc: pad:
                let
                  keybind = if pad.useMod then "${mod}+${pad.kb}" else "${pad.kb}";
                in
                (
                  acc
                  // {
          ...
          ++ (foldl' (
            acc: scratch: if scratch.autostart then acc ++ [ { command = "'${scratch.exec}'"; } ] else acc
          ) [ ] (attrValues scratchpads));
        };
        swaynag = {
          enable = true;
