          ...
          options = with lib; {
            ...
            __toString = mkOption {
              ...
              default =
                ...
                + (if self.resume != null then " resume ${std.escapeShellArg self.resume}" else "");
            };
          };
        }
      )
                    ...
                    evtsToStr =
                      ...
                      if event == "timeout" then
                        ...
                        (foldl' (
                          acc: cmd:
                          assert isString cmd;
                          acc + "\n${event} ${std.escapeShellArg cmd}"
                        ) "" entries);
                  in
                  foldl' (
                    acc: event:
                    if (event == "idlehint") then
                      acc + (std.optionalString (self.idlehint != null) "\nidlehint ${toString self.idlehint}")
                    else if event == "__toString" then
                      acc
                    else
                      acc + (evtsToStr event)
                  ) "" (attrNames self);
...
in
{
  options.desktop.wayland.idle = with lib; {
    enable = (mkEnableOption "wayland idle daemon") // {
      default = false;
    };
    settings = mkOption {
