...
in
{
  ...
  config = lib.mkIf (cfg.enable && idl.enable) {
    services.swayidle =
      ...
      {
        enable = true;
        package = if config.system.isNixOS then lib.signal.home.linkSystemApp pkgs { app = "swayidle"; } else pkgs.swayidle;
        ...
        events = [
          ...
          { event = "after-resume"; command = outputOn; }
        ];
        timeouts = [
          { timeout = 60; command = "if pgrep -x swaylock; then ${outputOff}; fi"; resumeCommand = outputOn; }
